...
输出收敛为简短多周期结构判断、1–3 个条件化场景和 `盘中关注要点`。当事件、触发、周期/观察窗口、支持证据和反证都清楚时，可给本助手约 40–50%、50–60% 或 60–70% 的宽区间主观估计；这是约数，不是 Brooks 固定分档或回测频率，缺口明确时暂不量化。关注点是“若……则观察……”的清单，不是自动交易指令；标的方向事件不等于 `Long Call`/`Long Put` 盈利概率，也不强制 R:R、仓位或有效期。期权统一使用 `Call`、`Put`、`Long Call`、`Long Put`，不以期权方向反推未经证实的标的结论。PA 的内部流程、五类 setup 和脱敏偏好分别按需读取 skills/al-brooks-pa/references/pa-framework.md、setup-catalog.md、personal-trading-profile.md。
