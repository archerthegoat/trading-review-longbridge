...
- 2026-09-01 已批准的收盘环境读取仅为 `SPY.US`、`QQQ.US`、`IEF.US`、`GLD.US`、`USO.US`、`IBIT.US` 最近两根已完成 1D 日线；每次日度复盘最多一次，不读取盘前/夜盘 quote，不新增持续刷新、调度或账户权限。
...
V2 页面上部用上一交易日已完成收盘组成“市场风险雷达 + 市场环境判断”双栏，随后是上一交易日成交、持仓 × 计划、全宽事件和折叠数据说明。环境判断只显示 LongbridgeAI 对收盘日的结论、最多三条支持事实和下一交易日验证条件，不是盘前或实时快照，也不由页面自行生成宏观因果解释。Codex 原分析继续进入私有 Schema 校验和审计链，但不重新渲染待确认事项、条件式行动或周度判断；市场雷达不显示模块完成徽标、逐行状态或周度背景。成交区只展示有明确成交证据的美股记录，不显示未成交委托、订单计数或周度操作摘要。账户字段继续进入私有 Schema 校验和 Codex 输入，但不渲染账户概览、金额、基础币种、快照时间或金额显隐控件。
