...
V2 页面上部用上一交易日已完成收盘组成“市场风险雷达 + 市场环境判断”双栏，随后是上一交易日成交、持仓 × 计划、全宽事件和折叠数据说明。环境判断只显示收盘结论、定价信号、跨资产确认和下一交易日观察，不是盘前或实时快照。Codex 原分析继续进入私有 Schema 校验和审计链，但不重新渲染待确认事项、条件式行动或周度判断；市场雷达不显示模块完成徽标、逐行状态或周度背景。成交区只展示有明确成交证据的美股记录，不显示未成交委托、订单计数或周度操作摘要。账户字段继续进入私有 Schema 校验和 Codex 输入，但不渲染账户概览、金额、基础币种、快照时间或金额显隐控件。
