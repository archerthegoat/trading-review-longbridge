...
SQLite v3 仍是事实、分析缓存、计划版本和周度 revision 的权威。采集运行继续按既有授权把原始响应放在 owner-only 临时目录，再白名单入库。现有数据库并未保存所有 UI 文案和展示范围声明，不能宣称 DB-only 可完整还原页面。
...
Obsidian Bridge 的实现范围已获批准，准确字段/模板门禁尚待确认，当前尚未启用正式同步；Web 本身不写 Vault。没有修改 Codex 调度。不能把常驻展示误当成每天自动生成新复盘；未来生成运行需显式调用 publish 才更新 `/`。
