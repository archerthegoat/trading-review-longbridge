...
3. 每日上部在完成收盘输入存在时使用“市场风险雷达 + 市场环境判断”双栏；判断明确绑定上一美股交易日收盘，不是盘前快照，也不随浏览器刷新。旧输入没有收盘环境对象时，市场雷达保持全宽兼容。Codex 原盘前分析仍保留在私有展示快照与审计链，不重新渲染待确认事项或行动面板。
...
成交卡前台按 canonical underlying 聚合为一标的一行；同一标的的动作标签按首次出现顺序去重，普通 ticker（包括工具类型为 unknown 的普通 ticker）只显示已核验的“买入”或“卖出”，不显示正股、单股杠杆 ETF 或工具待确认。期权仅保留机械确认且对用户有意义的最小标签，例如“买入 0DTE Call”“卖出 0DTE Put”；其他已知期权类型可显示方向与 Call/Put，不恢复具体合约身份。成交卡不显示计划状态、工具判断或“来源：…”字段，section note 固定为“按标的汇总实际成交”。底层结构化工具与计划字段继续用于审计和测试，不改变 Schema 或计划分类。
...
9. V1 全量测试继续通过。
10. 收盘环境双栏只消费固定六标的完成日线；夜盘数据不能进入新卡片，SPY/QQQ 缺失时不形成结论，Python/TS 输出保持一致。
